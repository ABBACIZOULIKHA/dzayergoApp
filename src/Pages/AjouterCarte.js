...
function AjouterCarte() {
  ...
  const [titre, setTitre] = useState("Point");
  const [SideDiv, setSidediv] = useState(0);

  const [categorie, setCategorie] = useState("");
  const categories = ["Musée", "Parc", "Monument"];

  const [theme, setTheme] = useState("");

  const themes = ["Histoir", "Loisir", "Nature"];
  const [horaire, setHoraire] = useState("");
  const [Description, setDescription] = useState("");
  const [transport, setTransport] = useState("");
  const [prix, setPrix] = useState("");

  const [listpoints, setListPoint] = useState([{ lat: 1, lng: 1 }]);

  const LieuCollectionRef = collection(db, "Lieu");
  const EnregPoint = async () => {
    try {
      await addDoc(LieuCollectionRef, {
        Categorie: categorie,
        HorOuv: horaire,
        Latitude: coord.lat,
      ...
      console.error(err);
      console.log(titre, theme, categorie, prix, coord.lng, coord.lat);
    }
  };
  ...
  return isLoaded ? (
    <div className=" w-full h-screen">
      <div className="h-[14%] bg-violet w-full flex flex-row justify-between items-center py-3 px-2">
        <Link to="/">
          {" "}
          <img src={logo} className=" ml-10  w-46 h-16" alt="" />{" "}
        </Link>
        ...
        <div className="flex flex-row justify-between space-x-5 ">
          <button
            ...
            className="bg-tchini hover:scale-110 text-white hover:font-semibold px-4 py-3 rounded-full "
          >
            {" "}
            Liste points interets{" "}
          </button>
          <button
            onClick={() => {
              setSidediv(1);
            }}
            className="bg-beige hover:scale-110 text-white hover:font-semibold  px-4 py-3 rounded-full "
          >
            {" "}
            Ajouter un point{" "}
          </button>
      ...
      <div
        id="Rechercher"
        className=" absolute z-10 flex justify-center items-center space-x-5 w-[40%] h-[6%] ml-[50%] mt-7  bg-white rounded-full drop-shadow-2xl"
      >
      ...
      {SideDiv === 1 ? (
        <div
          id="AjouterNvPoint"
          className=" absolute z-10 w-80 h-[80%] ml-5 mt-7  bg-white rounded-xl drop-shadow-2xl"
        >
          ...
          <div className="flex flex-col justify-center items-center space-y-2 mt-3">
            ...
            <button className="text-white bg-beige px-4 py-2 rounded-full text-xs mr-28">
              {" "}
              Ajouter un evenement{" "}
            </button>
            <p className="text-textgris text-xs mr-20">
              {" "}
              Veuillez remplir les informations{" "}
            </p>
            <div className="flex flex-row space-x-2 w-[80%] justify-between items-left">
              <button
                onClick={() => setSidediv(0)}
                className="text-white bg-tchini px-10 py-2 rounded-full text-xs"
              >
                {" "}
                Annuler{" "}
              </button>
              <button
                className="text-white bg-rose px-8 py-2 rounded-full text-xs"
                onClick={EnregPoint}
              >
                {" "}
                Enregistrer{" "}
